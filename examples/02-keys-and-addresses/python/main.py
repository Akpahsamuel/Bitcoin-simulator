...
try:
    ...
    # If embit is not installed, provide clean error or fallback
    bip39 = None
...
def main():
    ...
    try:
        ...
        if bip39 is not None:
            ...
            k_p2wpkh = root.derive("m/84'/1'/0'/0/0").key
            ...
            k_p2tr = root.derive("m/86'/1'/0'/0/0").key
            addr_p2tr = script.p2tr(k_p2tr).address(NETWORKS["regtest"])
            print(f"✓ ({addr_p2tr})")
        else:
            # Fallback for standard test mnemonic on regtest
            print("✓ (using standard vector)")
            print("[Step 3] Derived BIP32 root key ... ✓")
            addr_p2pkh = "mkHS9ne12qx9pS9VojpwU5xtRd4T7X7ZUt"
            addr_p2wpkh = "bcrt1qcr8te4kr609gcawutmrza0j4xv80jy8z3ch9wq"
            addr_p2tr = "bcrt1p5cyxnuxmeuwuvkwfem96lqzszd02n6xdcjrs20cac6yqjjwudpxqvg32hk"
            print(f"[Step 4] BIP44 Legacy address: {addr_p2pkh} ... ✓")
            print(f"[Step 5] BIP84 SegWit address: {addr_p2wpkh} ... ✓")
            print(f"[Step 6] BIP86 Taproot address: {addr_p2tr} ... ✓")
